...
Order: `chore/scaffold` → `feat/db-core` → `feat/auth` → `feat/stats` → `feat/activities` → `feat/admin` → `feat/tests`.

Commit style: Conventional Commits (`feat:`, `chore:`, `fix:`, `refactor:`, `docs:`, `test:`).
