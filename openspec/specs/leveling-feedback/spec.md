...
Enrich the `POST /activities/log` response with progress-to-next-level feedback at both the per-stat and global levels, so the client can render level-up feedback without recomputing thresholds — while preserving the existing level invariants and formula.
