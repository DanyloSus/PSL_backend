...
Serve a rich, filterable, paginated activity-history read for the authenticated user, and allow the user to delete their own logs with a single-transaction XP reversal that preserves the levels-never-decrease invariant. All access is authenticated; deletes are CSRF-guarded.
